...
For the full code demo, see the notes in the <a target="_blank" href="vizydrop_tooltips_fields.html" class="noCrossRef">tooltip demo</a>.

In this demo, the popovers pull in and display content from the information in a <a target="_blank" href="vizydrop_tooltips_source.json">vizydrop_tooltips_source.json</a> file located in the same directory.
...
Inside \_data > vizydrop create a YAML file called something like vizydrop_definitions.yml. Add the definitions for each of your tooltips here like this:
...
Create pages inside your new tooltips collection (that is, inside the \_tooltips folder). Each page needs to have a unique `id` in the frontmatter as well as a `product`. Then reference the definition you created in the vizydrop_definitions.yml file.
...
{{site.data.vizydrop.vizydrop_definitions.basketball}}
```
{%endraw%}
...
Inside your project's pages directory (e.g., vizydrop), add a file called "vizydrop_tooltips_source.json." (You can use whatever name you want.) Add the following to your JSON file:
...
Change "vizydrop" to the product name you used in each of the tooltip files. The template here will only include content in the JSON file if it meets the `product` attribute requirements. We need this `if` statement to prevent tooltips from other products from being included in the JSON file.
...
```
{
  "entries": [
    {
      "id": "baseball",
      "body": "{{site.data.vizydrop.vizydrop_definitions.baseball}}"
    },
    {
      "id": "basketball",
      "body": "{{site.data.vizydrop.vizydrop_definitions.basketball}}"
    },
    {
      "id": "football",
      "body": "{{site.data.vizydrop.vizydrop_definitions.football}}"
    },
    {
      "id": "soccer",
      "body": "{{site.data.vizydrop.vizydrop_definitions.soccer}}"
    }
  ]
}
```

You can also view the same JSON file here: <a class="noCrossRef" href="vizydrop_tooltips_source.json">vizydrop_tooltips_source.json</a>.
...
You can store your vizydrop_tooltips_source.json file anywhere you want, but to me it make sense to store it inside a tooltips folder for your specific project. This way it will automatically be excluded from other projects that are already excluding that project directory.
...
When you build your site, Jekyll will iterate through every page in your \_tooltips folder and put the page id and body into this format. In the output, look for the JSON file in the vizydrop/tooltips/vizydrop_tooltips_source.json file. You'll see that Jekyll has populated it with content. This is because of the triple hyphen lines in the JSON file &mdash; this instructs Jekyll to process the file.
...
```
curl -I http://idratherbetellingstories.com/wp-content/apidemos/vizydrop_tooltips_source.json
```
...
var url = "vizydrop_tooltips_source.json";
...
View the <a class="noCrossRef" href="tooltips/vizydrop_tooltips_fields.html">Tooltip Demo</a> for a demo.
...
Again, see the <a class="noCrossRef" href="tooltips/vizydrop_tooltips_fields.html">Tooltip Demo</a> for a demo of the full code.
...
View the source code of the <a class="noCrossRef" href="vizydrop_tooltips_fields.html">Tooltip Demo</a> for the full comments.
...
<tr>
<td>Basketball</td>
<td>{{site.data.vizydrop.vizydrop_definitions.basketball}}</td>
</tr>

<tr>
<td>Baseball</td>
<td>{{site.data.vizydrop.vizydrop_definitions.baseball}}</td>
</tr>

<tr>
<td>Football</td>
<td>{{site.data.vizydrop.vizydrop_definitions.football}}</td>
</tr>

<tr>
<td>Soccer</td>
<td>{{site.data.vizydrop.vizydrop_definitions.soccer}}</td>
</tr>
</tbody>
</table>
```
